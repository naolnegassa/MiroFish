"""
OASIS Twitter模拟预设脚本
此脚本读取配置文件中的参数来Execute模拟，实现全程自动化
...
使用方式:
    ...
    python run_twitter_simulation.py --config /path/to/simulation_config.json --no-wait  # Complete后立That isClose
"""
...
class IPCHandler:
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict]) -> bool:
        ...
        try:
            # 构建动作字典
            actions = {}
            agent_prompts = {}  # Log每itemsagent的prompt
            ...
            for interview in interviews:
                ...
                try:
                    ...
                    print(f"  Warning: no法GetAgent {agent_id}: {e}")
            
            if not actions:
                self.send_response(command_id, "failed", error="没yesyes效的Agent")
                return False
...
class TwitterSimulationRunner:
    ...
    def __init__(self, config_path: str, wait_for_commands: bool = True):
        ...
        Args:
            ...
            wait_for_commands: 模拟Complete后是否Waiting命令（默认True）
        """
    ...
    def _get_profile_path(self) -> str:
        """GetProfile文件路径（OASIS Twitter使用CSV格式）"""
    ...
    def _create_model(self):
        ...
        统一使用Project根目录 .env 文件中的配置（优先级最高）：
        - LLM_API_KEY: API密钥
        ...
        print(f"LLM configuration: model={llm_model}, base_url={llm_base_url[:40] if llm_base_url else '默认'}...")
    ...
    async def run(self, max_rounds: int = no):
        """运行Twitter模拟
        ...
        if max_rounds is not no and max_rounds > 0:
            ...
            if total_rounds < original_rounds:
                print(f"\nrounds数Completed截断: {original_rounds} -> {total_rounds} (max_rounds={max_rounds})")
        
        print(f"\nSimulation parameters:")
        print(f"  - 总模拟时长: {total_hours}小时")
        print(f"  - 每rounds时间: {minutes_per_round}分钟")
        ...
        # LoadAgent图
        print("LoadAgent Profile...")
        profile_path = self._get_profile_path()
        if not os.path.exists(profile_path):
            print(f"Error: Profile文件不存在: {profile_path}")
            return
        ...
        if os.path.exists(db_path):
            os.remove(db_path)
            print(f"CompletedDelete旧数据库: {db_path}")
        ...
        print("环境InitializeComplete\n")
        
        # InitializeIPCHandle器
        ...
        if initial_posts:
            print(f"Execute初始事件 ({len(initial_posts)}items初始帖子)...")
            initial_actions = {}
            for post in initial_posts:
                ...
                try:
                    ...
                    print(f"  Warning: no法为Agent {agent_id}Create初始帖子: {e}")
            
            if initial_actions:
                await self.env.step(initial_actions)
                print(f"  Completed发布 {len(initial_actions)} items初始帖子")
...
async def main():
    ...
    # 在 main 函数开始时Create shutdown 事件
...
def setup_signal_handlers():
    ...
    让程序yes机会正常清理资Source（Close数据库、环境等）
    """
