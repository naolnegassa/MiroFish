    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAG长短期记忆实时Update中' : '实时Update中...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">还yes少量内容Handle中，建议稍后手动RefreshGraph</span>
          <button class="hint-close-btn" @click="dismissFinishedHint" title="CloseNotice">
            ...
            <div class="detail-section" v-if="selectedItem.data.attributes && Object.keys(selectedItem.data.attributes).length > 0">
              ...
              <div class="properties-list">
                <div v-for="(value, key) in selectedItem.data.attributes" :key="key" class="property-item">
                  ...
                  <span class="property-value">{{ value || 'no' }}</span>
                </div>
              </div>
            </div>
...
const showSimulationFinishedHint = ref(false) // 模拟结束后的Notice
const wasSimulating = ref(false) // Trace之前是否在模拟中
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // 重置展开Status
}
...
const renderGraph = () => {
  ...
  // Log当前Handle到每对Nodes的第几items边
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // CompletedHandle的自环Nodes
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // 自环边 - 每itemsNodes只添加一items合and的自环
      if (processedSelfLoopNodes.has(e.source_node_uuid)) {
        return // CompletedHandle过，跳过
      }
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // 存储所yes自环边的详细Info
        }
      })
      return
    }
    ...
    // 判断边的方向是否and标准化方向一致（SourceUUID < TargetUUID）
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // 计算曲率：多items边时分散开，单items边为直线
    let curvature = 0
    if (totalCount > 1) {
      // 均匀分布曲率，确保明显区分
      ...
      // If边的方向and标准化方向相反，翻转曲率
      // 这样确保所yes边在同一参考系下分布，不会因方向不同but重叠
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation - 根据边数量动态调整Nodes间距
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // 根据这对Nodes之间的边数量动态调整距离
      // 基础距离 150，每多一items边增加 40
  ...
  const getLinkPath = (d) => {
    ...
    const offsetRatio = 0.25 + pairTotal * 0.05 // 基础25%，每多一items边增加5%
