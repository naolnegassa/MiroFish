    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            LLM分析文档内容andSimulation Requirement，提取出Real-world Seed，自动生成合适的本体结构
          </p>
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Based on生成的本体，将文档自动分块后调用 Zep 构建知识Graph，提取Entity和Relation，and形成时序记忆and社区摘要
          </p>
          ...
          <div class="stats-grid">
            <div class="stat-card">
              ...
              <span class="stat-label">EntityNodes</span>
            </div>
            <div class="stat-card">
              <span class="stat-value">{{ graphStats.edges }}</span>
              <span class="stat-label">Relation边</span>
            </div>
            <div class="stat-card">
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 2, 'completed': currentPhase >= 2 }">
        ...
        <div class="card-content">
          ...
          <p class="description">Graph ConstructionCompleted，请进入下一步进行模拟Environment Setup</p>
          <button 
            class="action-btn" 
          ...
          >
            ...
            {{ creatingSimulation ? 'Create中...' : '进入Environment Setup ➝' }}
          </button>
        </div>
      </div>
    </div>
...
// 进入Environment Setup - Create simulation and跳转
const handleEnterEnvSetup = async () => {
  if (!props.projectData?.project_id || !props.projectData?.graph_id) {
    console.error('缺少ProjectorGraphInfo')
    return
  }
  ...
  try {
    ...
    if (res.success && res.data?.simulation_id) {
      // 跳转到 simulation page面
      router.push({
        name: 'Simulation',
        params: { simulationId: res.data.simulation_id }
      })
    } else {
      console.error('Create模拟Failed:', res.error)
      alert('Create模拟Failed: ' + (res.error || '未知Error'))
    }
  } catch (err) {
    console.error('Create模拟异常:', err)
    alert('Create模拟异常: ' + err.message)
  } finally {
