    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Start中...' : '开始生成结果报告' }} 
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- 通用回退：未知Typeoryes content 但未被上述Handle -->
...
const runStatus = ref({})
const allActions = ref([]) // 所yes动作（增量累积）
const actionIds = ref(new Set()) // 用于去重的动作ID集合
const scrollContainer = ref(null)

// Computed
// 按时间顺序显示动作（最新的在最后面，That is底部）
...
// 格式化模拟流逝时间（根据rounds次和每rounds分钟数计算）
...
const resetAllState = () => {
  ...
  stopPolling()  // Stop之前可能存在的rounds询
}
...
const doStartSimulation = async () => {
  ...
  // 先重置所yesStatus，确保不会受到上一次模拟的影响
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('正在Start双平台and行模拟...')
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true,  // 强制重新开始
      enable_graph_memory_update: true  // 开Start态GraphUpdate
    }
    
    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`设置最大Simulation Rounds: ${props.maxRounds}`)
    }
    
    addLog('Completed开Start态GraphUpdate模式')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Completed清理旧的模拟日志，重新Start Simulation')
      }
      addLog('✓ 模拟引擎StartSuccess')
      ...
      startError.value = res.error || 'StartFailed'
      addLog(`✗ StartFailed: ${res.error || '未知Error'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ Start异常: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  addLog('正在Stop Simulation...')
  
  try {
    const res = await stopSimulation({ simulation_id: props.simulationId })
    
    if (res.success) {
      addLog('✓ 模拟CompletedStop')
      phase.value = 2
      stopPolling()
...
// Trace各平台的上一次rounds次，用于检测变化andOutput日志
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 检测模拟是否Completed（Through runner_status or平台CompleteStatus判断）
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // 额外Check：If后端还没来得及Update runner_status，但平台Completed经报告Complete
      // Through检测 twitter_completed 和 reddit_completed Status判断
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ 检测到所yes平台模拟Completed结束')
        }
        addLog('✓ 模拟Completed')
        phase.value = 2
        stopPolling()
    ...
    console.warn('Get运行StatusFailed:', err)
  }
}

// Check所yes启用的平台是否Completed
const checkPlatformsCompleted = (data) => {
  // If没yes任何平台数据，Back false
  if (!data) return false
  
  // Check各平台的CompleteStatus
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // If至少yes一items平台Complete了，Check是否所yes启用的平台都Complete了
  // Through actions_count 判断平台是否被启用（If count > 0 or running 曾为 true）
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // If没yes任何平台被启用，Back false
  if (!twitterEnabled && !redditEnabled) return false
  
  // Check所yes启用的平台是否都Completed
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    console.warn('Get详细StatusFailed:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Report Generation请求Completed发送，请稍候...')
    return
  }
  
  isGeneratingReport.value = true
  addLog('正在StartReport Generation...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Report Generation任务CompletedStart: ${reportId}`)
      
      // 跳转到报告page面
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ StartReport GenerationFailed: ${res.error || '未知Error'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ StartReport Generation异常: ${err.message}`)
    isGeneratingReport.value = false
  }
}
