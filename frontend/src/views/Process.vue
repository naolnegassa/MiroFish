<template>
  <div class="process-page">
    ...
    <div class="main-content">
      <!-- Left: Real-time knowledge graph display -->
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            ...
            <div class="action-buttons">
                ...
                <button class="action-btn" @click="toggleFullScreen" :title="isFullScreen ? 'Exit full screen' : 'Show full screen'">
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Graph visualization (show whenever data is available) -->
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">Graph building will start automatically after generation is complete</p>
          </div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After uploading documents, LLM analyzes the content and automatically generates ontology structure suitable for opinion simulation (entity types + relation types)
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API Description</div>
                <div class="detail-content">
                  Based on the generated ontology, chunk the documents and call Zep API to build the knowledge graph, extract entities and relations
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">Preparing to enter next step</div>
              </div>
...
const goToNextStep = () => {
  ...
  alert('Environment setup functionality is under development...')
}
...
// Initialize - Handle新建ProjectorLoadCompletedyesProject
const initProject = async () => {
  const paramProjectId = route.params.projectId
  
  if (paramProjectId === 'new') {
    // 新建Project：从 store Get待上传的数据
    await handleNewProject()
  } else {
    // LoadCompletedyesProject
    currentProjectId.value = paramProjectId
    await loadProject()
  }
}

// Handle新建Project - 调用 ontology/generate API
const handleNewProject = async () => {
  const pending = getPendingUpload()
  
  if (!pending.isPending || pending.files.length === 0) {
    error.value = '没yes待上传的文件，请Back首page重新Action'
    loading.value = false
    return
  }
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // Ontology Generation阶段
    ontologyProgress.value = { message: '正在上传文件and分析文档...' }
    ...
    if (response.success) {
      ...
      // 自动开始Graph Construction
      await startBuildGraph()
    } else {
      error.value = response.error || 'Ontology GenerationFailed'
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = 'ProjectInitializeFailed: ' + (err.message || '未知Error')
  } finally {
    loading.value = false
  }
}

// LoadCompletedyesProject数据
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || 'LoadProjectFailed'
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = 'LoadProjectFailed: ' + (err.message || '未知Error')
  } finally {
    loading.value = false
  }
}
...
const updatePhaseByStatus = (status) => {
  switch (status) {
    ...
    case 'graph_completed':
      ...
      break
    case 'failed':
      error.value = projectData.value?.error || 'HandleFailed'
      break
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: '正在StartGraph Construction...'
    }
    
    const response = await buildGraph({ project_id: currentProjectId.value })
    
    if (response.success) {
      buildProgress.value.message = 'Graph Construction任务CompletedStart...'
      
      // Save task_id 用于rounds询
      const taskId = response.data.task_id
      
      // StartGraph数据rounds询（独立于任务Statusrounds询）
      startGraphPolling()
      
      // Start任务Statusrounds询
      startPollingTask(taskId)
    } else {
      error.value = response.error || 'StartGraph ConstructionFailed'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'StartGraph ConstructionFailed: ' + (err.message || '未知Error')
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      buildProgress.value = {
        ...
        message: task.message || 'Handle中...'
      }
      
      console.log('Task status:', task.status, 'Progress:', task.progress)
      
      if (task.status === 'completed') {
        console.log('✅ Graph ConstructionComplete，正在Load完整数据...')
        
        stopPolling()
        stopGraphPolling()
        currentPhase.value = 2
        
        // Update进度显示为CompleteStatus
        buildProgress.value = {
          progress: 100,
          message: '构建Complete，正在LoadGraph...'
        }
        
        // 重新LoadProject数据Get graph_id
        const projectResponse = await getProject(currentProjectId.value)
        if (projectResponse.success) {
          ...
          if (projectResponse.data.graph_id) {
            console.log('📊 Load完整Graph:', projectResponse.data.graph_id)
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Graph ConstructionFailed: ' + (task.error || '未知Error')
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  simulation.on('tick', () => {
    // Update所yes边的位置（Include可见线和透明点击区域）
