...
// Get maxRounds from query parameters at initialization to ensure child component can access it immediately
...
const handleGoBack = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Environment not running, check if process needs to stop
...
const handleNextStep = () => {
  // Step3Simulation component handles report generation and route navigation
...
const loadGraph = async (graphId) => {
  // When simulating, do not show full-screen loading to avoid flickering
...
const startGraphRefresh = () => {
  ...
  // Refresh immediately, then refresh every 30 seconds
...
const stopGraphRefresh = () => {
  if (graphRefreshTimer) {
    ...
    addLog('Disabled real-time graph refresh')
  }
}
...
onMounted(() => {
  ...
  // Record maxRounds configuration (value already obtained from query parameter at initialization)
  if (maxRounds.value) {
